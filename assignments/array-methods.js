...
let ticketPriceTotal = runners.reduce(function(totalDonation, runners){
...
// Problem 1 We need to know how many people have the S size in order to place an immmediate order
...
// Problem 2 we need every runners email and first name pulled up in order to send email
  
runners.forEach(function(runners) {
    console.log(`${runners.first_name} ${runners.email}`);
   });

// Problem 3 Conver all names into lowercase

let allCaps = runners.map(function(currentValue){
    return `${currentValue.first_name.toLowerCase()} ${currentValue.last_name.toLocaleLowerCase()}`;
  });
  console.log(allCaps); 